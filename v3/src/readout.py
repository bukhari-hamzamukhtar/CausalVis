...
redirected the struck object.
    ...
    python readout.py --model v3_1_dynamics.pt --npz data/trajectories_out/sim_00077.npz --list
    python readout.py --model v3_1_dynamics.pt --npz data/trajectories_out/sim_00077.npz \
...
def main():
    ...
    for row in cols:
        ...
        hit_fact, tcol = pair_collides(qs_f, i, j, a.contact, a.thresh)
        if not hit_fact:
        ...
        if k in (i, j):
            ...
            hit_cf, _ = pair_collides(qs_c, i, j, a.contact, a.thresh)
        if hit_cf:
    ...
    if example is not None:
        ...
        if example["tag"] == "caused":
            ...
            print(f"   removing it, the {nm(p)} passes wide. The two distances above (contact "
                  f"{a.contact:.3f}) are the causal signature.")
        else:
