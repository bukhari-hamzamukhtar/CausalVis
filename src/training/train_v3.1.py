...
# ══════════════════════════════════════════════════════════════
# ROBUST PATH SETUP — makes the script location-independent
#
# This file lives at:      <PROJECT_ROOT>/src/training/train_v3.py
# hnn_gnn.py lives at:     <PROJECT_ROOT>/src/models/hnn_gnn.py
# the dataset lives at:    <PROJECT_ROOT>/data/causal_dataset_v2.pt(.bz2)
#
# We derive PROJECT_ROOT from this file's own location (not from
# whatever directory happens to be Colab's current working
# directory), so this works whether you `!python src/training/
# train_v3.py` from the repo root, `%cd` into src/training first,
# or run it via `%run`.
# ══════════════════════════════════════════════════════════════
try:
    THIS_DIR = os.path.dirname(os.path.abspath(__file__))
except NameError:
    # __file__ isn't defined if this were pasted directly into a
    # notebook cell rather than run as a .py file. Fall back to the
    # known Colab clone location.
    THIS_DIR = '/content/CausalVis/src/training'
...
# ══════════════════════════════════════════════════════════════
def main():
    DATASET_BZ2 = os.path.join(DATA_DIR, 'causal_dataset_v2.pt.bz2')
    DATASET_PT  = os.path.join(DATA_DIR, 'causal_dataset_v2.pt')
    SAVE_PATH   = os.path.join(PROJECT_ROOT, 'causal_gnn_v3_hnn.pt')
    BATCH_SIZE  = 64
    EPOCHS      = 50
    LR          = 0.005
    WD          = 1e-4
    ...
    LOG_PATH = os.path.join(PROJECT_ROOT, 'v3_training_log.pt')
    torch.save({
        'history':        history,
