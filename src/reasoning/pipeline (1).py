...
def _find_collision_for_objects(collisions, objects, subject, intervention):
    ...
    for i, event in enumerate(collisions):
        ...
        if intervention is None:
            # Zero-velocity style question: the SUBJECT itself is the
            # object to intervene on (no separate second object was
            # parsed). Must check WHICH index actually matched — a
            # previous version hardcoded `return i, 0` here regardless
            # of whether the subject matched o0 or o1, silently
            # intervening on the wrong object whenever the subject
            # happened to be stored at index 1.
            if matches(o0, subject):
                return i, 0
            elif matches(o1, subject):
                return i, 1
        else:
            if matches(o0, subject) or matches(o1, subject):
                if matches(o0, intervention):
                    return i, 0
                elif matches(o1, intervention):
                    return i, 1
    return 0, 0   # fallback: first collision


def ask(question: str, json_path: str, model, verbose: bool = True) -> dict:
    ...
    vprint("\n" + "█"*65)
    vprint("CAUSALVIS PIPELINE — FULL QUERY (V2 / PD_GNN)")
    vprint("█"*65)

    # ── Module I: Data Ingestion ──────────────────────────────
    vprint(f"\n[MODULE I] Loading: {os.path.basename(json_path)}")
    objects, collisions, positions = load_scene(json_path)
    vprint(f"  Objects: {len(objects)} | Collisions: {len(collisions)}")

    # ── Module III: Query Parser ──────────────────────────────
    vprint(f"\n[MODULE III] Parsing question...")
    vprint(f"  Q: \"{question}\"")
    parsed = parse_question(question)
    vprint(f"  Intervention type : {parsed['intervention_type']}")
    vprint(f"  Subject object    : {parsed['subject_object']}")
    vprint(f"  Intervention obj  : {parsed['intervention_object']}")
    vprint(f"  Functional tokens : {parsed['tokens']}")
    ...
    vprint(f"  Matched collision: Event {col_idx+1}, Frame {event['frame']}")
    vprint(f"  Target object idx: {target_idx} "
