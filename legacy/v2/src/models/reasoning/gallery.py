...
def run_gallery(data_dir, model, max_videos=30, events_per_video=None, seed=42):
    """
    ...
    events_per_video: how many collision EVENTS per video to generate
    questions for. Default None = ALL collisions in each video (not
    just the first) — testing only the first collision systematically
    under-samples later, often more decisive events, and produced a
    noticeably lower causal rate than the training-time CSR check in
    an earlier run. Set an integer to cap it if runtime matters more
    ...
    for fname in files:
        ...
        events_to_use = collisions if events_per_video is None else collisions[:events_per_video]
...
if __name__ == "__main__":
    ...
    gallery = run_gallery(DATA_DIR, model, max_videos=30, events_per_video=None)
