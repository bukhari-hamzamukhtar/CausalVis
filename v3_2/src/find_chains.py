...
away breaks it. The interaction filter (step 2) keeps out coincidences.
    ...
    python find_chains.py --data data/trajectories_out --model v3_2_dynamics.pt \
...
def main():
    ...
    ap.add_argument("--split", default=None, help="split.json -> scan a whole split")
    ap.add_argument("--which", default="test")
    ap.add_argument("--model", required=True)
    ap.add_argument("--contact", type=float, default=None,
                    help="override; default = measured per-pair r_i+r_j")
    ...
    if a.split:
        ...
        print(f"scanning the {a.which} split for chains")
    elif a.data:
        files = sorted(glob.glob(os.path.join(a.data, "*.npz")))
    else:
        raise SystemExit("give --split split.json --which test, or --data FOLDER")
    if a.limit:
        files = files[:a.limit]
    ...
    for fp in files:
        ...
        if len(cols) == 0:
            continue
        with torch.no_grad():
            _, _radii, _ = model.properties(strip_colour(
                torch.from_numpy(attrs.astype(np.float32)).unsqueeze(0)))
        _radii = _radii[0]
        def _contact(i, j):
            return a.contact if a.contact is not None else float(_radii[i] + _radii[j])
        ...
        for row in cols:
            ...
            hit, tcol = pair_collides(qs_f, i, j, _contact(i, j), a.thresh)
            if not hit:
                continue
            ...
            for k in range(N):
                if k in (i, j) or scene[3][k].item() == 0:
                    continue
                tki = first_contact_frame(qs_f, k, i, _contact(k, i), a.thresh)
                tkj = first_contact_frame(qs_f, k, j, _contact(k, j), a.thresh)
                interacted = (tki is not None and tki <= tcol) or (tkj is not None and tkj <= tcol)
                if not interacted:
                    continue

                qs_c, _ = simulate(model, do_remove(scene, k), steps)
                hit_c, _ = pair_collides(qs_c, i, j, _contact(i, j), a.thresh)
                if not hit_c:
                    ...
                    print(f"   reproduce: python src/readout.py --model {a.model} "
                          f"--npz <data>/{os.path.basename(fp)} --remove {k}\n")
