...
engine learned collision physics or just straight-line drift.
    ...
    python collision_check.py --model v3_2_dynamics.pt --npz data/trajectories_out/sim_00000.npz
"""
...
def main():
    ...
    ap.add_argument("--split", default=None, help="split.json -> evaluate a whole split")
    ...
    ap.add_argument("--contact", type=float, default=None,
                    help="override; default uses measured per-pair radii r_i+r_j")
    ...
    files = ([a.npz] if a.npz else None)
    if files is None:
        if not a.split:
            raise SystemExit("give --npz FILE or --split split.json --which test")
        from splits import load_split
        files = load_split(a.split, a.which)
        print(f"evaluating the {a.which} split: {len(files)} clips\n")

    grand_repro, grand_total = 0, 0
    for _fp in files:
        _one_file(model, _fp, a)
        grand_repro += _LAST["repro"]; grand_total += _LAST["total"]
    if len(files) > 1:
        print("=" * 70)
        rate = 100 * grand_repro / max(grand_total, 1)
        print(f"OVERALL on {a.which} split: {grand_repro}/{grand_total} "
              f"real collisions reproduced ({rate:.1f}%)")
        print("=" * 70)
    return

_LAST = {}
def _one_file(model, npz_path, a):
    ...
    radius = radius[0]
    def _contact(i, j):
        return a.contact if a.contact is not None else float(radius[i] + radius[j])
    ...
    print("=" * 70)
    _LAST["repro"] = reproduced; _LAST["total"] = len(cols)
